...
class T5Attention(nn.Module):
    ...
    #     relative_position = compute_invariant_position(query_length, key_length, positions)
    #     relative_position_bucket = self._relative_position_bucket(
        ...
        if position_bias is None:
            if not self.has_relative_attention_bias:
                ...
                if positions is not None:
                    position_bias = self.compute_invariant_bias(real_seq_length, key_length, positions)
                else:
...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def forward(
        ...
        if encoder_outputs is None:
            ...
            # TODO - apply order invariance positional bias to only encoder
            encoder_outputs = self.encoder(
                input_ids=input_ids,
                attention_mask=attention_mask,
                inputs_embeds=inputs_embeds,
        ...
        if self.constraint_ids is not None:
            lm_logits = self.enforce_constraints(lm_logits)
